...
for d in sys.argv[3:]:
    ...
    imp=band&(ff>=dQ-step); sky=band&(ff<0.02); print(f'{d:10}: band {int(band.sum())}; not behind occluder {int(imp.sum())} ({100*imp.mean()/max(1e-9,band.mean()):.1f} %); sky-valued {100*sky.sum()/band.sum():.1f} %; sky-valued in lower third {100*(sky&low).sum()/max(1,(band&low).sum()):.1f} % of {int((band&low).sum())}; fill median {np.median(ff[band]):.3f}')
