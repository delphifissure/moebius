...
def build(domains, label):
    ...
    if selfVal is not None and label == 'stop':
        # S35 §33: the surface's own continuation competes like a fitted sheet -- behind the texel by more than the tolerance, nearest wins
        okS = np.isfinite(selfVal) & (selfVal < ownD - ownT); d = np.flatnonzero(okS); v = selfVal[d]
        b = best[d]; upd = v > b; second[d[upd]] = np.maximum(second[d[upd]], b[upd]); best[d[upd]] = v[upd]; who[d[upd]] = -2
        second[d[~upd]] = np.maximum(second[d[~upd]], v[~upd])
        print(f'   [{label}] self-continuation wins {int(upd.sum())} of {len(d)} candidate texels')
...
selfVal = None
if A.selfcont:
    # S35 §33, SELF-CONTINUATION, the rest-ray form. The plug at band texel b stands for what lies BEHIND b along its rest ray.
    # Walk from b along a grid line while on b's own joined surface; it ends at t_s onto a surface X of disparity D_X (the frame
    # counts as the sky, D_X = 0). The point behind b is X. From an eye displaced sideways, a texel of b's own surface at distance
    # t occludes X's point iff KPAR (D_t - D_X) <= t (its parallax excess over X is used up within its distance). If that happens
    # before the surface ends, b's own surface covers b's rest position with itself: SELF, and the fill is the surface's next
    # depth level behind b. If the surface ends first, X is revealed behind b: the sheets model it, nothing is added. The
    # milkmaid's body (KPAR dD to the wall ~ 600 texels, silhouette 50 away) reveals the wall; starwatcher's near plain (100 rows
    # to a far plain a small step down) occludes it with itself. Two screen-space forms were tried first and are falsified by
    # the kit: 'the first same-surface level behind' filled every figure with itself; 'the first crossing KPAR (D_b - D_j) >= j'
    # filled L3's curved milkmaid and S2's slabs with themselves (L3 0.000 -> 0.083 m, S2 0.000 -> 0.056 m).
    tsc = time.time(); _cj = (compJ if A.patches else comp).ravel(); Dr = DISP.ravel(); Tr = TOL.ravel(); bIdx = np.flatnonzero(band.ravel()); selfVal = np.full(N, -np.inf)
    bx0 = bIdx % pw; by0 = bIdx // pw; D0 = Dr[bIdx]; T0_ = Tr[bIdx]; g0 = _cj[bIdx]; JMAX = 400
    # Each grid direction is one eye motion of the envelope; the truth counts a reveal from ANY eye, so the surface continues
    # itself at b only when NO direction reveals X. A walk that ends at the frame is the surface continuing (not a silhouette); an
    # exit onto a NEARER surface says nothing about what is behind b. (The first combination, nearest self candidate over the
    # directions, kept the milkmaid: her skirt 135 texels below her chest occludes the wall for a vertical eye motion while a
    # sideways one reveals it 50 texels away.)
    revealed = np.zeros(len(bIdx), bool); bestBehind = np.full(len(bIdx), -np.inf)
    for dx_, dy_ in DIRS:
        alive = np.ones(len(bIdx), bool); firstBehind = np.full(len(bIdx), -np.inf); mExcess = np.full(len(bIdx), np.inf); DX = np.full(len(bIdx), np.nan); frameEnd = np.zeros(len(bIdx), bool)
        for j_ in range(1, JMAX + 1):
            xx_ = bx0 + j_ * dx_; yy_ = by0 + j_ * dy_; inb = (xx_ >= 0) & (xx_ < pw) & (yy_ >= 0) & (yy_ < ph)
            ii_ = np.where(inb, yy_ * pw + xx_, 0); Dj = Dr[ii_]; same = (_cj[ii_] == g0)
            leave = alive & (~inb | ~same); DX[leave & inb] = Dj[leave & inb]; frameEnd[leave & ~inb] = True; alive &= ~leave
            if not alive.any(): break
            fb = alive & (D0 - Dj > T0_) & ~np.isfinite(firstBehind); firstBehind[fb] = Dj[fb]
            mExcess[alive] = np.minimum(mExcess[alive], KPAR * Dj[alive] - j_)
        farExit = np.isfinite(DX) & (DX < D0)                                   # the surface ended onto something farther: X is behind b
        revealed |= farExit & ~(mExcess <= KPAR * np.nan_to_num(DX))            # and no own texel occludes X's point before the surface ends
        bestBehind = np.maximum(bestBehind, firstBehind)
    selfHere = ~revealed
    selfVal[bIdx[selfHere]] = np.where(np.isfinite(bestBehind[selfHere]), bestBehind[selfHere], D0[selfHere] - T0_[selfHere] * 1.001); nSelf = int(selfHere.sum())
    # the decision per SURFACE by majority of its band texels: a small blob of another group on a plain breaks the walk for the
    # texels behind it and left salt-and-pepper sky (starwatcher, 16 % of the plain's band)
    valid_ = np.isfinite(selfVal[bIdx]) & (selfVal[bIdx] < D0 - T0_)
    gB = np.bincount(g0, minlength=int(_cj.max()) + 1); gS = np.bincount(g0[valid_], minlength=int(_cj.max()) + 1)
    selfGrp = gS > 0.5 * gB; fill_ = selfGrp[g0] & ~valid_
    selfVal[bIdx[fill_]] = D0[fill_] - T0_[fill_] * 1.001
    nsc = int(np.isfinite(selfVal[bIdx]).sum()); print(f'self-continuation: {nsc} of {len(bIdx)} band texels ({100*nsc/max(1,len(bIdx)):.1f} %) continue themselves ({nSelf} by no reveal in any direction, {int(selfGrp.sum())} surfaces by majority, {int(fill_.sum())} texels filled by the majority); depth median {float(np.median(depth_of_disp(selfVal[bIdx][np.isfinite(selfVal[bIdx])]))) if nsc else float("nan"):.3f} vs own {float(np.median(dQ.ravel()[bIdx][np.isfinite(selfVal[bIdx])])) if nsc else float("nan"):.3f}  ({time.time()-tsc:.1f}s)')
results = {}
