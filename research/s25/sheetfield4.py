#!/usr/bin/env python3
"""Sprint 17c (from Sprint 15): MODE=plate — the clamped plate (biharmonic energy) per RUN CLUSTER with Cauchy data on the far rim (the run's window, raw values), free edge under the occluder, the ground as an obstacle; MODE=amle — the infinity-Laplacian extension (Oberman 2005 scheme) from the same Cauchy strip. R4 §3, §4, §6. Sprint 15 text follows.
...
# ---------------------------------------------------------------- Sprint 17c: 2-D fields per run cluster (R4 §3 plate, §4 AMLE, §6 obstacle)
U2 = None
if MODE in ('plate', 'amle'):
    tP = time.time()
    # 1. every candidate's data, per free texel and side (the per-line law's own quantities)
    Cj = np.full((N, 2), -1, np.int64); Cp = np.zeros((N, 2), np.int64); Cw = np.zeros((N, 2), np.int64); Ca = np.zeros((N, 2), np.int64); Cb = np.zeros((N, 2), np.int64); Cthin = np.ones((N, 2), bool); Cv = np.full((N, 2), np.nan)
    for i in np.nonzero(free)[0]:
        ax = axis[i]
        for s4, dirn in ((0, -1), (1, 1)):
            j = rimJ[i, s4]
            if j < 0 or sky[j]: continue
            c = candidate(i, j, ax, dirn, rimW[i, s4])
            Cj[i, s4] = j; Cp[i, s4] = c['p']; Cw[i, s4] = c['w']; Ca[i, s4] = c['a']; Cb[i, s4] = c['b']; Cthin[i, s4] = c['thin']; Cv[i, s4] = c['v']
    # 2. run clusters: the used rim texels, 4-adjacent AND joined (the join law) -> connected components (the fold segmentation)
    used = np.zeros(N, bool); used[Cj[Cj >= 0]] = True; ui = np.nonzero(used)[0]
    rows_, cols_ = [], []
    # two rim texels are in one cluster iff they lie in the SAME RUN along the axis that connects them (row runs for x-neighbours,
    # column runs for y-neighbours): the run segmentation breaks at creases in both directions, so a wall and the floor it
    # stands on are separate clusters although their depth is continuous across the crease (the join law alone merges them —
    # Sprint 12's failure (1)).
    for j in ui:
        x_, y_ = j % pw, j // pw
        if x_ + 1 < pw and used[j + 1] and RS[0][j] == RS[0][j + 1] and RE[0][j] == RE[0][j + 1]: rows_.append(j); cols_.append(j + 1)
        if y_ + 1 < ph and used[j + pw] and RS[1][j] == RS[1][j + pw] and RE[1][j] == RE[1][j + pw]: rows_.append(j); cols_.append(j + pw)
    nK, clus = connected_components(coo_matrix((np.ones(len(rows_)), (rows_, cols_)), shape=(N, N)), directed=False)
    clus = np.where(used, clus, -1)
    # 3. per cluster: the Cauchy strip (each rim's run window, raw disparity, fixed) and the reach (the free texels it serves)
    strip = {}; reach = {}
    for i in np.nonzero(free)[0]:
        ax = axis[i]; st = 1 if ax == 1 else pw
        for s4, dirn in ((0, -1), (1, 1)):
            j = Cj[i, s4]
            if j < 0 or Cthin[i, s4]: continue
            K = clus[j]; reach.setdefault(K, []).append(i); sd = strip.setdefault(K, {})
            p = Cp[i, s4]; w = Cw[i, s4]; a = Ca[i, s4]; b = Cb[i, s4]
            for k in range(int(w)):   # from the rim into the visible side, within the run: the window the per-line law fits
                pos = p - k if dirn < 0 else p + k
                if pos < a or pos > b: break
                n = j + (pos - p) * st; sd[n] = D[n]
    U2 = np.full((N, 2), np.nan); nSolved = 0; nIter = 0; nObst = 0
    def solve_cluster(K):
        global nIter, nObst
        R_ = np.array(sorted(set(reach[K])), np.int64); S_ = strip.get(K, {})
        if len(S_) < 2: return None
        Sidx = np.fromiter(S_.keys(), np.int64); Sval = np.fromiter(S_.values(), float)
        dom = np.concatenate([R_, Sidx]); dom = np.unique(dom)
        xs, ys = dom % pw, dom // pw; x0, x1, y0, y1 = xs.min(), xs.max(), ys.min(), ys.max(); bw, bh = x1 - x0 + 1, y1 - y0 + 1
        inD = np.zeros((bh + 2, bw + 2), bool); inD[ys - y0 + 1, xs - x0 + 1] = True
        u = np.full((bh + 2, bw + 2), np.nan); fixed = np.zeros((bh + 2, bw + 2), bool)
        u[Sidx // pw - y0 + 1, Sidx % pw - x0 + 1] = Sval; fixed[Sidx // pw - y0 + 1, Sidx % pw - x0 + 1] = True
        # initial guess: the per-line values (either side)
        for s4 in (0, 1):
            m_ = ~np.isnan(Cv[R_, s4]); u[R_[m_] // pw - y0 + 1, R_[m_] % pw - x0 + 1] = np.where(np.isnan(u[R_[m_] // pw - y0 + 1, R_[m_] % pw - x0 + 1]), Cv[R_[m_], s4], u[R_[m_] // pw - y0 + 1, R_[m_] % pw - x0 + 1])
        u = np.where(np.isnan(u) & inD, np.nanmean(Sval), u)
        gB = np.full((bh + 2, bw + 2), -np.inf)
        if G:
            gx = np.arange(x0 - 1, x1 + 2); gy = np.arange(y0 - 1, y1 + 2); GX, GY = np.meshgrid(gx, gy); okc = (GX >= 0) & (GX < pw); gcol = np.zeros_like(okc); gcol[okc] = gCol[GX[okc]]
            gB = np.where(gcol, ground_at(GX, GY), -np.inf)
        tolD = np.full((bh + 2, bw + 2), np.inf); tolD[ys - y0 + 1, xs - x0 + 1] = tol[dom]
        if MODE == 'amle':
            # Oberman's monotone scheme: u <- (max + min over the 8-neighbourhood inside the domain) / 2, fixed texels held, obstacle by max
            free2 = inD & ~fixed
            for it in range(20000):
                ...
                for dy in (-1, 0, 1):
                    for dx in (-1, 0, 1):
                        ...
                        sh = np.roll(np.roll(u, dy, 0), dx, 1); shm = np.roll(np.roll(inD, dy, 0), dx, 1); pads.append(np.where(shm, sh, np.nan))
                P = np.stack(pads); mx = np.nanmax(P, 0); mn = np.nanmin(P, 0)
                un = np.where(free2, 0.5 * (mx + mn), u); un = np.where(free2 & (un < gB - tolD), gB, un)   # obstacle: never behind the ground
                ch = np.nanmax(np.abs(un - u)[free2]) if free2.any() else 0.0; u = un
                if ch < 1e-4 * np.nanmin(tolD[inD]): break
            nIter += it + 1
        else:
            # the clamped plate: minimise the discrete bending energy over the domain with the strip fixed; every other boundary natural (free edge)
            idx = -np.ones((bh + 2, bw + 2), np.int64); unk = inD & ~fixed
            for rnd in range(4):
                ids = np.nonzero(unk); nU = len(ids[0]); idx[:] = -1; idx[ids] = np.arange(nU)
                if nU == 0: break
                rowsA, colsA, valsA, rhs = [], [], [], []; r = 0
                def add(cells, wts):
                    nonlocal r
                    b_ = 0.0; anyU = False
                    for (cy, cx), wt in zip(cells, wts):
                        if unk[cy, cx]: rowsA.append(r); colsA.append(idx[cy, cx]); valsA.append(wt); anyU = True
                        else: b_ -= wt * u[cy, cx]
                    if anyU: rhs.append(b_); r += 1
                    else: pass
                Y, X = np.nonzero(inD)
                for cy, cx in zip(Y, X):
                    if inD[cy, cx - 1] and inD[cy, cx + 1]: add(((cy, cx - 1), (cy, cx), (cy, cx + 1)), (1.0, -2.0, 1.0))
                    if inD[cy - 1, cx] and inD[cy + 1, cx]: add(((cy - 1, cx), (cy, cx), (cy + 1, cx)), (1.0, -2.0, 1.0))
                    if inD[cy, cx + 1] and inD[cy + 1, cx] and inD[cy + 1, cx + 1]: add(((cy, cx), (cy, cx + 1), (cy + 1, cx), (cy + 1, cx + 1)), (1.4142135, -1.4142135, -1.4142135, 1.4142135))
                if r == 0: break
                A = csr_matrix((valsA, (rowsA, colsA)), shape=(r, nU)); bvec = np.array(rhs)
                if nU <= 60000: sol = spsolve((A.T @ A).tocsc(), A.T @ bvec)
                else: res = lsqr(A, bvec, atol=1e-12, btol=1e-12, iter_lim=20000, x0=u[ids]); sol = res[0]; nIter += res[2]
                u[ids] = sol
                below = unk & (u < gB - tolD)   # obstacle: a texel that came out behind the ground is fixed at the ground and the plate re-solved
                if not below.any(): break
                u[below] = gB[below]; unk &= ~below; nObst += int(below.sum())
        for s4 in (0, 1):
            m_ = (Cj[R_, s4] >= 0)
            m_ &= np.array([clus[j] == K if j >= 0 else False for j in Cj[R_, s4]])
            U2[R_[m_], s4] = u[R_[m_] // pw - y0 + 1, R_[m_] % pw - x0 + 1]
        return len(dom)
    for K in sorted(reach.keys(), key=lambda k: -len(reach[k])):
        if solve_cluster(K) is not None: nSolved += 1
    print(f'[{MODE}] run clusters {len(set(clus[ui].tolist()))} (used rims {len(ui)}), solved {nSolved} with reach; iterations {nIter}; obstacle texels {nObst}; {time.time()-tP:.1f}s')
def field2d_value(i, s4):
    v = U2[i, s4] if U2 is not None else np.nan
...
for i in np.nonzero(free)[0]:
    ...
    for s4, dirn in ((0, -1), (1, 1)):
        ...
        if MODE != 'own' and not c['thin'] and not (v == ground_at(i % pw, i // pw) if (G and gCol[i % pw]) else False):
            sv = regularised(i, s4, c) if MODE in ('slope', 'cmed', 'both') else (field2d_value(i, s4) if MODE in ('plate', 'amle') else surface_value(i, c, (comp[i], sj)))
            if sv is not None:
                gB = ground_at(i % pw, i // pw) if (G and gCol[i % pw]) else -np.inf
