        ...
        for line in range(nl):
            ...
            for tt in range(1, L + 1):
                ...
                if brk:
                    ...
                    s0 = tt
t0 = time.time(); segment_all(); print(f'runs segmented as the app does ({time.time()-t0:.1f}s)')
...
for i in np.nonzero(free)[0]:
    ...
    for s4, dirn in ((0, -1), (1, 1)):
        ...
        if MODE != 'own' and not c['thin'] and not (v == ground_at(i % pw, i // pw) if (G and gCol[i % pw]) else False):
            sv = surface_value(i, c, (comp[i], sj))
            if sv is not None:
...
np.save(f'{d}/sf2_{MODE}_depth.npy', newDepth.astype(np.float32))
