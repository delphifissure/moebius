...
PICS = ['bristlecone', 'octopus', 'room', 'silverwarrior', 'starwatcher', 'vermeer']; ARMS = ['da3', 'da3f', 'repo8', 'repo8inv', 'da3sky']
rows = []
for P in PICS:
    for arm in ARMS:
        d = A + f'b_{P}_{arm}'; ui = U + f'ub_{P}_{arm}'
        if not os.path.exists(d + '/meta.json'): continue
        sec = logpart(LOG, f'probe {P} {arm}'); r = parse(sec); m = json.load(open(d + '/meta.json')); pw, ph = m['pw'], m['ph']; N = pw * ph
...
cols = [('picture', 'pic'), ('arm', 'arm'), ('plate', 'size'), ('sheets', 'sheets'), ('far-side texels', 'far'), ('thin / cand', None), ('layer 2', 'l2'), ('band %', 'band_pct'), ('carriers %', 'car_pct'),
        ('band 15/25/35/45', None), ('torn', 'torn'), ('seams', 'seams'), ('clones (colour / final)', None), ('interior holes on the path (alpha 0 not touching the picture edge; offsets 0.05/0.1/0.2/0.301,0.068/0.26,0.088 m)', 'holes'), ('edge-connected alpha 0', 'dark'), ('quantum', 'q')]
